"""
    Model of an in-line electric heater booster.
"""
        ...
        for eid, esim in self.simulators.items():
            ...
            for attr, incoming in data.items():
                ...
                if attr in self.input_vars:
                    ...
                    if 'T' in attr:
                        newval = mean(val for val in incoming.values())  # [Don't] Convert degC from co-simulation flow to degK
                        setattr(esim, attr, newval)
                    elif 'mdot' in attr:
                        newval = mean(-val for val in incoming.values())  # Reverse the sign of incoming mass flow values
                        setattr(esim, attr, newval)
                    else:
        ...
        for eid, esim in self.simulators.items():
            ...
            for attr in requests:
                ...
                if attr in self.input_vars or attr in self.output_vars:
                    ...
                    if 'T' in attr:
                        mydata[attr] = getattr(esim, attr)  # Don't Convert local degK to degC for sending into the co-simulation flow
                    else:
